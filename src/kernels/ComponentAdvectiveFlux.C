...
ComponentAdvectiveFlux::ComponentAdvectiveFlux(const InputParameters & parameters) :
  Kernel(parameters),
...
void ComponentAdvectiveFlux::computeResidual()
{
  upwind(true, false, 0.);
}
...
void ComponentAdvectiveFlux::computeJacobian()
{
   upwind(false, true, _var.number());
}

Real ComponentAdvectiveFlux::computeQpOffDiagJacobian(unsigned int jvar)
{
...
void ComponentAdvectiveFlux::computeOffDiagJacobian(unsigned int jvar)
{
   upwind(false, true, jvar);
}

void ComponentAdvectiveFlux::upwind(bool compute_res, bool compute_jac, unsigned int jvar)
{
